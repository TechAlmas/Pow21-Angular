...
export class DispensaryComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe((params) => {
      ...
      if (this.user_data && this.user_data["email"]) {
        ...
        this.cookieService.get("_mio_user_email") &&
        this.cookieService.get("_mio_user_email") != ""
      ) {
        this.checkUser = true;
      }
  ...
  getDispensaryDetail(disp_slug) {
    this.getDispensaryDetailData(disp_slug).subscribe(
      (data: any) => {
        ...
        this.image_disp = this.dispDetails.logoUrl;
        this.mapUrl =
          "https://www.google.com/maps/place/" +
          this.dispDetails.address +
          "," +
          this.dispCity +
          "," +
          this.dispState +
  ...
  review_check_email() {
    return this._http.get<any[]>(
      "review_check_email?email=" + $(document).find(".reviewEmail").val()
    );
  }
  ...
  onSubmitReviewForm(form: NgForm) {
    ...
    if (!this.isUserLoggedIn) {
      ...
      this.cookieService.get("_mio_user_id") &&
      this.cookieService.get("_mio_user_id") != ""
    ) {
      this.review.user_id = parseInt(this.cookieService.get("_mio_user_id"));
    }
      ...
      this.postReview().subscribe(
        ...
        () => {
          if (this.review_id > 0) {
            this.review_id = 0;
            toastr.success(
              '<i class="icon-warning-sign"></i>&nbsp;&nbsp; Awesome! The POW Team has received your review. If it meets the community guidelines, it will be published momentarily.  ',
              "",
              {
                closeButton: true,
                timeOut: "8000",
  ...
  followStore(id) {
    if (this.user_data) {
      ...
      this.disp_follow(this.fav_disp).subscribe(
        (data) => {
          if (data["data"] == "follow") {
            ...
            $(".tooltip-contentfollow span.count").html(
            ...
            toastr.warning(
              '<i class="icon-warning-sign"></i>&nbsp;&nbsp;You are no longer following&nbsp;' +
      ...
      this.cookieService.set(
        "fav_disp",
        JSON.stringify(this.fav_disp),
        this.expiredDate,
        "/"
      );
      toastr.success(
        '<i class="icon-ok-sign"></i>&nbsp;&nbsp;Awesome, you have followed the store',
        "",
        {
          closeButton: true,
          timeOut: "8000",
      ...
      $(".tooltip-contentfollow span").html(
        parseInt($(".tooltip-contentfollow span").text()) + 1
      );
  ...
  onLogin(form: NgForm) {
    ...
    this.postLogin().subscribe(
      (data) => {
        ...
        if (data["api_status"] == 1) {
          toastr.success(
            "<i class='icon-ok-sign'></i>&nbsp;&nbsp;Login sucess fully !",
            "",
            {
              closeButton: true,
              timeOut: "7000",
          ...
          toastr.error(
            "<i class='icon-ok-sign'></i>&nbsp;&nbsp;Please provide valid details !",
            "",
            {
              closeButton: true,
              timeOut: "7000",
